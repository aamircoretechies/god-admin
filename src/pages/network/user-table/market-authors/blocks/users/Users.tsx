  ...
  const data: IUsersData[] = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      filtered = filtered.filter((user) =>
        ...
        user.user.description?.toLowerCase().includes(searchLower) ||
        user.team.label.toLowerCase().includes(searchLower)
      );
    }

    // Apply status filter (using team or rating as status indicator)
    if (statusFilter !== 'all') {
      // For now, we'll filter based on rating or other criteria
      // You can adjust this based on your data structure
    }
