    ...
    const loadPrompts = async () => {
      ...
      try {
        const response = await fetchPrompts({
          ...
          category:
            categoryFilter !== 'all' ? convertCategoryToApiFormat(categoryFilter) : undefined,
  ...
  const handleToggleStatus = async (promptId: string, currentStatus: string) => {
    ...
    try {
      ...
      if (response.status === 1) {
        ...
        toast.success(
          `Prompt ${newStatus === 'Active' ? 'activated' : 'deactivated'} successfully`
        );
      } else {
      ...
      const errorMessage =
        err?.response?.data?.message || err?.message || 'Failed to update prompt status';
      toast.error(errorMessage);
    }
  };
      ...
      {Math.ceil(totalCount / pageSize) > 1 && (
        <div className="flex items-center justify-between p-4 border-t">
          <div className="text-sm text-gray-600">
            Showing page {currentPage} of {Math.ceil(totalCount / pageSize)} ({totalCount} total
            prompts)
          </div>
